"""PrivateRDP v0.2.0 RELAY-LOCK ULTRA60 native Windows desktop host.
...
PrivateRDP signaling bridge. It never opens an Internet-facing port.
"""
...
LOG = logging.getLogger("private-rdp-host")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")

HOST = "127.0.0.1"
PORT = int(os.environ.get("RDP_DESKTOP_IPC_PORT", "8765"))
TARGET_W = max(640, int(os.environ.get("RDP_DESKTOP_WIDTH", "1600")))
TARGET_H = max(360, int(os.environ.get("RDP_DESKTOP_HEIGHT", "720")))
REQUESTED_DESKTOP_W = max(640, int(os.environ.get("RDP_DESKTOP_OS_WIDTH", "1600")))
REQUESTED_DESKTOP_H = max(360, int(os.environ.get("RDP_DESKTOP_OS_HEIGHT", "720")))
TARGET_FPS = max(15, min(60, int(os.environ.get("RDP_DESKTOP_FPS", "60"))))
ENABLE_AUDIO = os.environ.get("RDP_DESKTOP_AUDIO", "0").strip().lower() in {"1", "true", "yes", "on"}
...
if sys.platform != "win32":
    raise RuntimeError("rdp_webrtc_host.py is Windows-only")
...
class DesktopHost:
    def __init__(self) -> None:
        ...
        self.active_ice_provider = os.environ.get("RDP_ICE_PROVIDER", "none")
        self.active_ice_mode = os.environ.get("RDP_ICE_MODE", "relay-only")
    ...
    def rtc_config(self, offered_ice: Any = None, offered_provider: str = "", offered_mode: str = "", relay_preference: str = "auto") -> RTCConfiguration:
        ...
        if servers:
            ...
            raw = os.environ.get("RDP_ICE_SERVERS_JSON", "").strip()
            if raw:
                try:
                    servers = self._parse_ice_items(json.loads(raw), self.active_relay_transport)
                except Exception as exc:
                    LOG.warning("RDP_ICE_SERVERS_JSON parse failed: %s", exc)
            self.active_ice_provider = os.environ.get("RDP_ICE_PROVIDER", "none")
            self.active_ice_mode = os.environ.get("RDP_ICE_MODE", "relay-only")
        ...
        turn_count = sum(1 for x in schemes if x.startswith("turn"))
        LOG.info("ICE servers loaded provider=%s mode=%s relayTransport=%s count=%d stun=%d turn=%d sessionOverride=%s hostAutoStunFast=%s",
                 ...
                 sum(1 for x in schemes if x.startswith("stun")), turn_count,
                 bool(offered_ice), host_auto_stun_fast)
        if "relay" in self.active_ice_mode.lower() and turn_count < 1:
            raise RuntimeError("PrivateRDP RELAY-LOCK requires a TURN server")
    ...
    async def new_pc(self, session_id: int, offered_ice: Any = None, offered_provider: str = "", offered_mode: str = "", relay_preference: str = "auto") -> RTCPeerConnection:
        ...
        if self.audio is not None:
            ...
            LOG.info("remote audio disabled (RDP_DESKTOP_AUDIO=0) for lower overhead / normal Android media routing")
        ...
        def on_datachannel(channel):
            ...
            def on_open():
                ...
                asyncio.create_task(self.send({
                    ...
                    "desktopProtocol": 4, "sessionId": pc_session,
    ...
    async def apply_control(self, obj: dict[str, Any]) -> None:
        ...
        presets = {
            "survival": (1600, 720, 30),
            "low": (1600, 720, 40),
            "balanced": (1600, 720, 50),
            "high": (1600, 720, 60),
        }
        ...
        target_bitrate_kbps = max(1000, min(10000, int(obj.get("targetBitrateKbps", 4500) or 4500)))
        max_bitrate_kbps = max(target_bitrate_kbps, min(10000, int(obj.get("maxBitrateKbps", 10000) or 10000)))
        ...
        await self.send({
            ...
            "targetBitrateKbps": target_bitrate_kbps, "maxBitrateKbps": max_bitrate_kbps,
    ...
    async def _send_offer_ack(self, key: tuple[int, int, int]) -> None:
        sid, nid, rev = key
        await self.send({
            "type": "rtc-offer-ack", "desktopProtocol": 4,
    ...
    async def _handle_offer(self, obj: dict[str, Any], offer_generation: int, key: tuple[int, int, int]) -> None:
        ...
        try:
            ...
            answer_payload = {"type": "rtc-answer", "desktopProtocol": 4, "sessionId": sid,
            ...
            await self.send({
                ...
                "desktopProtocol": 4, "sessionId": sid,
                "captureAvailable": bool(self.video and self.video.capture_available),
    ...
    async def client(self, ws) -> None:
        ...
        await self.send({"type": "desktop-state", "state": "host-ready", "desktopProtocol": 4, "sessionId": self.session_id, "turnMode": self.active_ice_mode, "iceProvider": self.active_ice_provider, "relayTransport": self.active_relay_transport})
...
async def main() -> None:
    ...
    async with websockets.serve(host.client, HOST, PORT, max_size=4 * 1024 * 1024, ping_interval=10, ping_timeout=20):
        LOG.info("PrivateRDP v0.1.0 ULTRA60 desktop IPC ready ws://%s:%d max=%dx%d@%dfps touch=%s", HOST, PORT, TARGET_W, TARGET_H, TARGET_FPS, _TOUCH_OK)
